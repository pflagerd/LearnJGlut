		...
		singleShotDisplayTest(() -> {
			...
			for (double radius = circle_spacing; (radius < 1.1); radius = ( radius +circle_spacing)) {
				...
				for (double i = 0; i < circle_points; i++) { 
			    	...
			    	glColor3f(0f, 0f, 1f);
					glVertex2f((float)Math.cos(Math.toRadians(k)) * IncrementalRadius, (float)Math.sin(Math.toRadians(k)) * IncrementalRadius);
				}
			} 
			glEnd();
		...
		singleShotDisplayTest(() -> {
			...
			for (double radius = circle_spacing; (radius < 1.1); radius = ( radius +circle_spacing)) {
				...
				for (double i = 0; i < circle_points; i++) { 
					...
					k = k + 360 / (double) circle_points; //Increments Spacing between points
			    	glColor3f(0f, 0f, 1f);
					glVertex2f(Math.cos(Math.toRadians(k)) * IncrementalRadius, (float)Math.sin(Math.toRadians(k)) * IncrementalRadius);
				}
			} 
			glEnd();
			glFinish(); // waits for display to settle down.
